...
function Conversation({ turns, saying, stage, running }: ConversationProps) {
    ...
    <div className="flex flex-col gap-3">
      ...
      {running && (
        <div className="text-muted-foreground flex items-center gap-2 px-1 text-xs">
          ...
          {/* Until a stage is reported there is nothing to name, and the spinner alone says
              the true thing: it is working, and nobody has said what it is doing. */}
          {stage === null ? t("ai.conversation.working") : t(`ai.stage.${stage}`)}
        </div>
      )}
    </div>
  );
