...
//!
//! It reaches the development network alone. The command line refuses it beside the production
//! network before anything starts, because a node whose clock somebody can move is a node that
//! signs roots for hours that have not happened.
//!
...
mod tests {
    ...
    impl Scratch {
        fn new(name: &str) -> Self {
            let path = std::env::temp_dir().join(format!("almena-cli-clock-{name}"));
